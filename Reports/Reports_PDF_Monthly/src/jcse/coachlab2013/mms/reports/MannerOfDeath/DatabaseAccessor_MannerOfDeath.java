...
public class DatabaseAccessor_MannerOfDeath extends Template_DatabaseAccessor {
    ...
    /**
     * creates an sql statement, executes it and returns results as a results
     * set.
     *
     * @return tempSet
     */
...
"WHERE EXTRACT(MONTH FROM TIMESTAMP (`reporting_Body`.dateInserted))=MONTH(NOW());");
    ...
    public ResultSet read(String deathType) {
        ResultSet tempSet = null;
        ...
        try {
            ...
            preparedStatement = connection.prepareStatement("SELECT\n"
                    ...
                    + "	WHERE `etl_MannerOfDeath`.`deathType` = ?;");

            preparedStatement.setString(1, deathType);
            tempSet = preparedStatement.executeQuery();
